...
if(MOR_tanh)
    ...
    Ws = 600*m1km*(1); % width of the mid-ocean ridge
    Xs = 350*m1km;
    etab= - hmax + h_ridge/2 - h_ridge/2 .*tanh( (X-Xs)/Ws*4 );
end
...
[z3D_r,Cs_r] = mor.zlevs4(-h_rho,0*h_rho,theta_s,theta_b,hc,Nlayer,'r','new2012');
[z3D_w,Cs_w] = mor.zlevs4(-h_rho,0*h_rho,theta_s,theta_b,hc,Nlayer,'w','new2012');
...
[mask_u, mask_v, mask_psi] = mor.uvp_masks(mask_rho);
...
if(use_randForc)
    ...
    % Total number of forcing snapshots (e.g., daily forcing for ~1 years)
    Nfrc = 365*5; % +1 to account for leap years
    ...
    if(restoring)
        Lramp_RF = 30*m1km;
        L_nudg_wind = L_nudg - Lramp_RF;

        RF_mask_u = ones(size(mask_u));
        idx_0 = (X_u <= L_nudg_wind );
        idx_cos = (X_u >= L_nudg_wind   & X_u < L_nudg_wind +Lramp_RF);
        RF_mask_u(idx_0) = 0;
        RF_mask_u(idx_cos) = 0.5.*( 1-cos(pi*(X_u(idx_cos)-L_nudg_wind )/Lramp_RF));

        idx_0 = (X_u >= Lx-L_nudg_wind );
        idx_cos = (X_u >= Lx-L_nudg_wind -Lramp_RF  & X_u < Lx-L_nudg_wind  );
        RF_mask_u(idx_0) = 0;
        RF_mask_u(idx_cos) = 0.5.*( 1-cos(pi*(X_u(idx_cos)-(Lx-L_nudg_wind ))/Lramp_RF));

        RF_mask_v = ones(size(mask_v));
        idx_0 = (X_v <= L_nudg_wind );
        idx_cos = (X_v >= L_nudg_wind   & X_v < L_nudg_wind +Lramp_RF);
        RF_mask_v(idx_0) = 0;
        RF_mask_v(idx_cos) = 0.5.*( 1-cos(pi*(X_v(idx_cos)-L_nudg_wind )/Lramp_RF));

        idx_0 = (X_v >= Lx-L_nudg_wind );
        idx_cos = (X_v >= Lx-L_nudg_wind -Lramp_RF  & X_v < Lx-L_nudg_wind  );
        RF_mask_v(idx_0) = 0;
        RF_mask_v(idx_cos) = 0.5.*( 1-cos(pi*(X_v(idx_cos)-(Lx-L_nudg_wind ))/Lramp_RF));
    ...
    %% have a look on wind stress

    crange4 =  1.5*[-RF_F0_rot RF_F0_rot];  % for RFrot_alltime
    fontsize = 18;
    LW = 1;
    ...
    for n = 1 %:360
        ...
        % Plot RFrot_alltime as background color
        pcolor(X_v / 1000, Y_v / 1000, tauy_v(:,:,n));
        hold on
        clim(crange4)
        cmocean('red', 100, 'pivot', 0)
        shading interp
        ...
        ylim([0 Ly] / 1000)
        colorbar('Position',[0.915 0.13 0.01 0.775],'tickLabelinterpreter', 'latex')
        title(['Random force tauy; Day ' num2str(n)  ], 'FontSize', fontsize+2, 'Interpreter', 'latex');
        % Save figure
        folderName = './RF';
        if ~exist(folderName, 'dir')
            mkdir(folderName)
        end
        exportgraphics(gcf, [folderName, '/tauy_', num2str(n), '.png'], 'Resolution', 200)
    end
...
elseif(use_wind)
    ...
    tauy_v = 0*Y_v;
    Lwind = Ws;
    idx = and(X_v > L_nudg, X_v <=L_nudg+Lwind);
    tauy_v(idx) = - tau0*sin(pi*(X_v(idx)-L_nudg)/Lwind).^2;
    ...
    tau_x_rho = mor.u2rho(taux_u);
    tau_y_rho = mor.v2rho(tauy_v);
    figure
...
if(restoring)
    ...
    nudg_mask_2d(idx_l,:) =( (L_nudg-(X(idx_l,:)))./L_nudg ) .* mask_rho(idx_l,:);
    nudg_mask_3d = repmat(nudg_mask_2d,[1 1 Nlayer]);
    uv_nudgc = nudg_mask_3d .* 1/nudg_Time;
    ...
    figure('Position',[50 50 800 800]);
    pcolor(X / 1000, Y / 1000, squeeze(nudgc(:,:,10)));
    shading flat
    colorbar
    cmocean('shad',100)
    axis equal
    ...
    xlabel('X (km)','Interpreter','Latex');
    ylabel('Y (km)','Interpreter','Latex');
    title('temperature nudging coefficient s$^{-1}$','Interpreter','Latex')
    xlim([0 Lx]/1000)
    ylim([0 Ly]/1000)
    exportgraphics(gcf,'nudgc.png','resolution',200)




    figure('Position',[50 50 800 800]);
    pcolor(X / 1000, Y / 1000, squeeze(uv_nudgc(:,:,10)));
    shading flat
    colorbar
    cmocean('shad',100)
    axis equal
    ...
    xlabel('X (km)','Interpreter','Latex');
    ylabel('Y (km)','Interpreter','Latex');
    title('momentum nudging coefficient s$^{-1}$','Interpreter','Latex')
    xlim([0 Lx]/1000)
    ylim([0 Ly]/1000)
    exportgraphics(gcf,'uv_nudgc.png','resolution',200)
...
if(restoring)
    ...
    varid_uv_nudgc = netcdf.defVar(ncid, 'uv_nudgc', 'NC_DOUBLE', [dimid_xi_rho, dimid_eta_rho, dimid_s_rho]);
    netcdf.putAtt(ncid, varid_uv_nudgc, 'long_name', 'Nudging coefficient');
    netcdf.putAtt(ncid, varid_uv_nudgc, 'units', 'second-1');
...
generate_BasicDiag_file( ...
    true, 365*s1day, 1, ...   % restart: enabled, period, nrpf
    true, s1day, 100, ...     % snapshots: enabled, period, nrpf
    true, 365*s1day, ndig );      % averages:  enabled, period, nrpf

% Extended diagnostics (averaged; set booleans as desired)
generate_MoreDiag_file( ...
    true, 365*s1day, ndig, ...   % diag_avg, output_period, nrpf
